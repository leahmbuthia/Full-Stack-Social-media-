...
SELECT * FROM Event LEFT JOIN EventAttendee ON Event.EventID =EventAttendee.EventID;
SELECT Event.*, EventAttendee.*, tbl_user.Username, tbl_user.TagName
FROM Event
LEFT JOIN EventAttendee ON Event.EventID = EventAttendee.EventID
LEFT JOIN (
    SELECT EventAttendee.AttendeeID, tbl_user.Username, tbl_user.TagName  
    FROM EventAttendee 
    INNER JOIN tbl_user ON EventAttendee.AttendeeID = tbl_user.UserID
) AS tbl_user ON EventAttendee.AttendeeID = tbl_user.AttendeeID;
